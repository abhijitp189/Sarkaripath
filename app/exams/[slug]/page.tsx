...
export default function ExamDetailPage({ params }: { params: { slug: string } }) {
  ...
  // If we have a detailed entry, render the full page
  if (detailed) {
    return <DetailedExamPage exam={detailed} />;
  }

  // Otherwise render the basic page from allExams data
  return <BasicExamPage exam={brief!} />;
}

// ─── DETAILED EXAM PAGE (5 exams with full content) ─────────────────────────
function DetailedExamPage({ exam }: { exam: ReturnType<typeof getExamBySlug> & {} }) {
  const sections = [
  ...
  return (
    <>
          ...
          <div>
            ...
            <section id="eligibility" className="mb-12">
              ...
              <div className="card p-6">
                ...
                <div className="mb-3 text-sm text-surface-600">
                  Minimum: <strong>{exam.minAge} years</strong> · Maximum: <strong>{exam.maxAge} years</strong>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {exam.ageRelaxations.map((ar: any, i: number) => (
                        <tr key={i} className="border-t border-surface-100"><td className="p-3 font-medium text-surface-800">{ar.category}</td><td className="p-3 text-surface-600">{ar.relaxation}</td></tr>
                      ))}
                    </tbody>
                  </table>
                </div>
            ...
            <section id="exam-pattern" className="mb-12">
              ...
              <div className="space-y-4">
                {exam.examStages.map((stage: any, i: number) => (
                  <div key={i} className="card p-6">
                    ...
                    <div className="space-y-1.5">
                      {stage.subjects.map((sub: string, j: number) => (
              ...
              <div className="space-y-4">
                {exam.syllabus.map((section: any, i: number) => (
                  <details key={i} className="card group" open={i === 0}>
                    ...
                    <div className="px-5 pb-5 ml-9 space-y-2">
            ...
            <section id="books" className="mb-12">
              ...
              <div className="card overflow-hidden"><div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="bg-surface-50"><th className="text-left p-4 font-semibold text-surface-700">Book</th><th className="text-left p-4 font-semibold text-surface-700">Author</th><th className="text-left p-4 font-semibold text-surface-700">Subject</th><th className="text-left p-4 font-semibold text-surface-700">Free</th></tr></thead>
                  <tbody>
                    {exam.bestBooks.map((book: any, i: number) => (
                      <tr key={i} className="border-t border-surface-100">
                        <td className="p-4 font-medium text-surface-800">{book.title}</td><td className="p-4 text-surface-600">{book.author}</td>
                        <td className="p-4"><span className="badge badge-primary">{book.subject}</span></td>
                        <td className="p-4">{book.freeLink ? <a href={book.freeLink} target="_blank" rel="noopener noreferrer" className="text-primary-500 font-medium">Free →</a> : <span className="text-surface-400">Buy online</span>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-24">
              ...
              <div className="card p-5 mt-4 bg-primary-50 border-primary-200">
                ...
                <a href={exam.officialWebsite} target="_blank" rel="noopener noreferrer" className="text-sm text-primary-600 hover:text-primary-700 break-all">{exam.officialWebsite.replace('https://', '')} →</a>
              </div>
            </div>
          </aside>
        </div>
      ...
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(generateBreadcrumbJsonLd([{ name: 'Home', url: '/' }, { name: 'Exams', url: '/exams' }, { name: exam.shortName, url: `/exams/${exam.slug}` }])) }} />
    </>
  );
}

// ─── BASIC EXAM PAGE (95 exams with CSV data) ───────────────────────────────
function BasicExamPage({ exam }: { exam: ReturnType<typeof getExamBriefBySlug> & {} }) {
  return (
    <div className="container-main py-10">
      <nav className="text-sm text-surface-500 mb-6">
        <Link href="/" className="hover:text-primary-500">Home</Link>
      ...
      <div className="max-w-4xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-10">
          <div className="bg-surface-50 rounded-xl p-4 border border-surface-200">
            <div className="text-xs text-surface-400 uppercase tracking-wide">Conducting Body</div>
            <div className="font-semibold text-surface-800 mt-1 text-sm">{exam.conductingBody}</div>
          </div>
          <div className="bg-surface-50 rounded-xl p-4 border border-surface-200">
            <div className="text-xs text-surface-400 uppercase tracking-wide">Age Limit</div>
            <div className="font-semibold text-surface-800 mt-1 text-sm">{exam.minAge} – {exam.maxAge === 99 ? 'No limit' : exam.maxAge + ' years'}</div>
          </div>
          <div className="bg-surface-50 rounded-xl p-4 border border-surface-200">
            <div className="text-xs text-surface-400 uppercase tracking-wide">Qualification</div>
            <div className="font-semibold text-surface-800 mt-1 text-sm">{exam.qualification}</div>
          </div>
          {exam.salaryRange ? (
            <div className="bg-surface-50 rounded-xl p-4 border border-surface-200">
              <div className="text-xs text-surface-400 uppercase tracking-wide">Salary Range</div>
              <div className="font-semibold text-emerald-600 mt-1 text-sm">{exam.salaryRange}</div>
            </div>
          ) : (
            <div className="bg-surface-50 rounded-xl p-4 border border-surface-200">
              <div className="text-xs text-surface-400 uppercase tracking-wide">Frequency</div>
              <div className="font-semibold text-surface-800 mt-1 text-sm">{exam.frequency}</div>
            </div>
          )}
        </div>
        ...
        <section className="mb-10">
          <h2 className="text-xl font-heading font-bold text-surface-900 mb-4 flex items-center gap-2">
            <span className="w-7 h-7 bg-primary-100 rounded-lg flex items-center justify-center text-primary-600 text-xs">1</span>
            Eligibility Criteria
          </h2>
          <div className="card p-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              <div>
                <h3 className="font-semibold text-surface-800 mb-1">Educational Qualification</h3>
                <p className="text-surface-600">{exam.qualification}</p>
              </div>
              <div>
                <h3 className="font-semibold text-surface-800 mb-1">Age Limit (General Category)</h3>
                <p className="text-surface-600">{exam.minAge} – {exam.maxAge === 99 ? 'No upper limit' : exam.maxAge + ' years'}</p>
              </div>
              {exam.avgVacancies && (
                <div>
                  <h3 className="font-semibold text-surface-800 mb-1">Approximate Vacancies</h3>
                  <p className="text-surface-600">{exam.avgVacancies}</p>
                </div>
              )}
              <div>
                <h3 className="font-semibold text-surface-800 mb-1">Level</h3>
                <p className="text-surface-600">{exam.level} Government</p>
              </div>
            </div>
            <div className="mt-4 pt-4 border-t border-surface-100">
              <h3 className="font-semibold text-surface-800 mb-2">Standard Age Relaxation</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                <div className="bg-surface-50 rounded-lg p-2"><span className="text-surface-400">OBC</span><div className="font-medium text-surface-700">+3 years</div></div>
                <div className="bg-surface-50 rounded-lg p-2"><span className="text-surface-400">SC/ST</span><div className="font-medium text-surface-700">+5 years</div></div>
                <div className="bg-surface-50 rounded-lg p-2"><span className="text-surface-400">PwBD</span><div className="font-medium text-surface-700">+10 years</div></div>
                <div className="bg-surface-50 rounded-lg p-2"><span className="text-surface-400">Ex-SM</span><div className="font-medium text-surface-700">+5 years</div></div>
              </div>
              <p className="text-xs text-surface-400 mt-2">* Relaxation may vary. Always check the official notification for exact details.</p>
            </div>
          </div>
        </section>
        ...
        <section className="mb-10">
          <h2 className="text-xl font-heading font-bold text-surface-900 mb-4 flex items-center gap-2">
            <span className="w-7 h-7 bg-primary-100 rounded-lg flex items-center justify-center text-primary-600 text-xs">2</span>
            Selection Process
          </h2>
          <div className="card p-6">
            <div className="flex flex-wrap gap-3">
              {exam.selectionProcess.split(',').map((stage, i) => (
                <div key={i} className="flex items-center gap-2">
                  ...
                  <span className="bg-primary-50 text-primary-700 px-3 py-2 rounded-lg text-sm font-medium">{stage.trim()}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="mb-10">
          <h2 className="text-xl font-heading font-bold text-surface-900 mb-4 flex items-center gap-2">
            <span className="w-7 h-7 bg-primary-100 rounded-lg flex items-center justify-center text-primary-600 text-xs">3</span>
            Official Website & Links
          </h2>
          <div className="card p-6">
            <a href={`https://${exam.officialWebsite}`} target="_blank" rel="noopener noreferrer" className="text-primary-500 hover:text-primary-600 font-medium text-lg break-all">
              {exam.officialWebsite} →
            </a>
            <p className="text-sm text-surface-500 mt-2">Visit the official website for the latest notifications, admit cards, results, and application forms.</p>
          </div>
        </section>
        ...
        <div className="text-xs text-surface-400 bg-surface-50 rounded-lg p-4">
          <strong>Disclaimer:</strong> The information provided is for general guidance. Exam patterns, syllabus, and eligibility criteria may change. Always refer to the official notification on {exam.officialWebsite} for the latest and most accurate information.
        </div>
      </div>
    </div>
  );
